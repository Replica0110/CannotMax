...
class ArknightsApp:
	def __init__(self, root):
		...
		# 添加行号显示和跳转功能
		self.row_label = tk.Label(root, text="当前行号: 0 / 总行数: 0")
		...
		# 跳转到开头按钮
		self.first_button = tk.Button(root, text="跳到开头", command=self.jump_to_first_row)
		self.first_button.pack(side=tk.LEFT, padx=5)

		# 跳转到结尾按钮
		self.last_button = tk.Button(root, text="跳到结尾", command=self.jump_to_last_row)
	...
	def show_row(self, row_index):
		...
		# 更新行号显示
		self.row_label.config(text=f"当前行号: {row_index + 1} / 总行数: {len(self.data)}")
	...
	def jump_to_first_row(self):
		"""跳转到第一行"""
		if self.data:
			self.current_row_index = 0
			self.show_row(self.current_row_index)

	def jump_to_last_row(self):
		"""跳转到最后一行"""
		if self.data:
